...
public:
      ...
      strand_(strand)
  {
    std::cout << __PRETTY_FUNCTION__ << std::endl;
		...
		boost::asio::ip::address addr = socket_.remote_endpoint().address();
		if(addr.is_v6())
		{
			std::cout<<"remote_endpoint: ipv6 address: ";
		}
		else
    {
			std::cout<<"remote_endpoint: ipv4 address: ";
    }
  ...
  void start()
  {
    ...
    do_read();
      ...
      long timeout_request_ = 5;
      /// Timeout on content handling. Defaults to 300 seconds.
      long timeout_content_ = 300;
      /// Maximum size of request stream buffer. Defaults to architecture maximum.
      /// Reaching this limit will result in a message_size error code.
      std::size_t max_request_streambuf_size_ = std::numeric_limits<std::size_t>::max();
    ...
    socket_.async_read_some(boost::asio::buffer(data_),
        ...
        boost::asio::bind_executor(strand_,
          [this, self](boost::system::error_code ec, std::size_t length)
          {
            ...
            if (ec == boost::asio::error::eof) //connection closed by router
            {
...
public:
  ...
  //auto ip = boost::asio::ip::make_address(host_name);
